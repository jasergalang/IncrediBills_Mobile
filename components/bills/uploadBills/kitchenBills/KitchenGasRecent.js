...
export default function KitchenGasRecent({ kitchenGasBills, removeUpload }) {
      ...
      <View className="flex-row items-center justify-between mb-4">
        <View>
          ...
          <Text className="text-sm text-slate-600">
            {kitchenGasBills.length} files uploaded
          </Text>
        </View>
        ...
        {kitchenGasBills.map((bill) => {
          ...
          return (
            <TouchableOpacity
              key={bill._id}
              onPress={() =>
                navigation.navigate("KitchenGasBillDetails", { id: bill._id })
              }
