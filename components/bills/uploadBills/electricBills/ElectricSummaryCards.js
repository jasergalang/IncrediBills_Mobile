...
export default function ElectricSummaryCards({ electricBills }) {
  const totalUploads = electricBills?.bills?.length || 0;
  const processedCount = electricBills?.bills?.filter(b => b.status === "Success").length || 0;
      ...
      <View className="flex-row gap-3 mb-6">
        <View className="flex-1">
          ...
          >
            ...
            <Text className="text-2xl font-bold text-amber-600">{totalUploads}</Text>
            <Text className="text-xs text-green-600 font-semibold mt-1">
