...
export default function TransportRecent({ transportBills, removeUpload }) {
      ...
      <View className="flex-row items-center justify-between mb-4">
        <View>
          ...
          <Text className="text-sm text-slate-600">
            {transportBills.length} files uploaded
          </Text>
        </View>
        ...
        {transportBills.map((bill) => {
          ...
          return (
            <TouchableOpacity
              key={bill._id}
              onPress={() =>
                navigation.navigate("TransportBillDetails", { id: bill._id })
              }
