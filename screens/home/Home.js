  ...
  const statsData = useMemo(() => {
    ...
    Object.values(allBills).forEach((categoryBills) => {
      categoryBills.forEach((bill) => {
        ...
        if (!latestDate || billDate > latestDate) latestDate = billDate;
      });
    });
    ...
    categoriesWithLatestMonth.forEach((key) => {
      ...
      if (match) predictedTotal += match.predictedCost || 0;
    });

    // ✅ Compute real total saved from allSavings (same logic as Analytics)
    let totalSaved = 0;
    Object.values(allSavings).forEach((savingsArr) => {
      if (!Array.isArray(savingsArr)) return;
      savingsArr.forEach((saving) => {
        totalSaved += saving.savedCost || 0;
      });
    });
    ...
    return {
      ...
      savedAmount: Math.round(totalSaved), // ✅ real saved from savings API
      savedChange: predictionChange,
    };
  }, [bills.allBills, predictions, allSavings]); // ✅ add allSavings as dependency
  // const statsData = useMemo(() => {
  //   const allBills = bills.allBills;
  ...
  const latestPerCategoryBills = useMemo(() => {
    ...
    Object.entries(allBills).forEach(([key, categoryBills]) => {
      ...
      const sorted = [...categoryBills].sort(
        (a, b) =>
          new Date(b.billMonth || b.date) - new Date(a.billMonth || a.date)
      );
