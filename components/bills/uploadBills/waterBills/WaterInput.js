...
export default function WaterInput({
  ...
  const providers = ["Manila Water", "Maynilad"];
  ...
  const onSelectBillingStart = (day) => {
    ...
    // if end is before start, move end to same day
  ...
  return (
    <View className="bg-white rounded-2xl p-4 mb-4 shadow-sm">
      ...
      {useManualEntry ? (
        // Manual entry inputs: date, cost, consumption, billingPeriod, provider, status, feedback
        <>
          <View className="mb-4">
              ...
              <View className="bg-white border border-slate-200 rounded-xl mt-2 overflow-hidden shadow-lg">
                {paymentStatuses.map((status) => (
                  ...
                  >
                    ...
                    {paymentStatus === status && <Ionicons name="checkmark" size={20} color="#3b82f6" />}
                  </TouchableOpacity>
                ))}
              </View>
            )}
          ...
          {/* Feedback */}
          <View className="mb-4">
            <Text className="text-sm font-medium text-slate-600 mb-2">Feedback / Notes <Text className="text-slate-400">(optional)</Text></Text>
            <TextInput
              className="bg-slate-50 rounded-xl px-4 py-3 text-slate-800 border border-slate-200"
        ...
        // OCR/upload mode: only payment status + feedback (image handled by parent UploadBox)
        <>
          <View className="mb-4">
            <Text className="text-sm font-medium text-slate-600 mb-2">Payment Status <Text className="text-red-500">*</Text></Text>
            <TouchableOpacity
              className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-200 flex-row items-center justify-between"
              onPress={() => setShowDropdown(!showDropdown)}
            >
      ...
      >
        <View className="flex-row items-center">
          ...
          <Text className="text-white font-semibold text-base">{isSubmitting ? "Submitting..." : "Submit Bill"}</Text>
        </View>
      </TouchableOpacity>
    </View>
  );
