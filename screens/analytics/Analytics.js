...
export default function Analytics({ navigation }) {
  ...
  const utilityKPI = useMemo(() => {
    if (!allBills || Object.keys(allBills).length === 0) {
    ...
    Object.entries(allBills).forEach(([category, bills]) => {
      ...
      bills?.forEach((bill) => {
        // bills use billMonth; fall back to date
        const raw = bill.billMonth || bill.date;
        if (!raw) return;
    ...
    // Compute savings — iterate allSavings directly, not via allBills loop
    let currentMonthSavings = 0;
    let totalAllTimeSavings = 0;

    Object.entries(CATEGORY_TO_SAVINGS_KEY).forEach(([billCategory, savingsKey]) => {
      if (!selectedCategories.includes("all") && !selectedCategories.includes(billCategory)) return;
      const savingsArr = allSavings?.[savingsKey];
      if (!savingsArr || savingsArr.length === 0) return;

      savingsArr.forEach((saving) => {
        // savings objects have a top-level "month" field — use that first
        const raw = saving.month || saving.date || saving.createdAt;
        if (!raw) return;
  ...
  const spendingData = useMemo(() => {
    return monthly.map((m) => {
      ...
      return {
        ...
        water, electricity, miscellaneous, transport, groceries, kitchenGas,
  ...
  const { categoryBreakdown, categoryTotalAmount } = useMemo(() => {
    const totals = { electricity: 0, water: 0, grocery: 0, fuel: 0, miscellaneous: 0, kitchenGas: 0 };
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        {activeTab === "utility" ? (
          <>
            ...
            <CategoryBreakdown categoryBreakdown={categoryBreakdown} totalAmount={categoryTotalAmount} />
          </>
        ) : (
          <>
