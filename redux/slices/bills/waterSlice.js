...
export const updateWaterBill = createAsyncThunk(
  "water/update",
  async ({ billId, updatedData }, { dispatch, rejectWithValue }) => {
    try {
      const data = await updateWaterBillApi(billId, updatedData);
      dispatch(fetchWaterBills());
      return data;
    } catch (err) {
      const errorMessage =
...
const waterSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ── updateWaterBill ────────────────────────────────────────────────────
      .addCase(updateWaterBill.pending, (state) => {
        state.updating = true;
        state.error = null;
      })
      .addCase(updateWaterBill.fulfilled, (state) => {
        state.updating = false;
        state.error = null;
      })
      .addCase(updateWaterBill.rejected, (state, action) => {
        state.updating = false;
        state.error = action.payload;
      })
...
export const { removeWaterBillLocal, clearRecommendations, clearWaterError } =
  waterSlice.actions;
