...
export default function Home({ navigation }) {
  ...
  // SAFE FETCH FUNCTION FOR PREDICTIONS
  const fetchSafePredictions = async (url, token, label = "") => {
    try {
      ...
      console.log(`${label} predictions missing — using fallback defaults.`);
      return { predictions: [] }; // safe fallback
    }
  };
  ...
  const homeData = async () => {
    ...
    try {
      // FETCH MAIN BILLS
      const electricRes = await axios.get(`${baseURL}/api/electric-bill/all`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });

      const waterRes = await axios.get(`${baseURL}/api/water-bill/all`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });

      // SAFELY FETCH PREDICTIONS
      const electricPredRes = await fetchSafePredictions(
        `${baseURL}/api/electric-bill/predictions`,
        user.token,
        "Electricity"
      );

      const waterPredRes = await fetchSafePredictions(
        `${baseURL}/api/water-bill/predictions`,
        user.token,
        "Water"
      );
      ...
      const electricPredictions = electricPredRes.predictions || [];
      const waterPredictions = waterPredRes.predictions || [];
      ...
      const latestAmounts = {
        electricity:
          latestElectric?.cost ??
          totals.electricity ??
          utilities.find((u) => u.id === "electricity")?.amount ??
          0,
        water:
          latestWater?.cost ??
          totals.water ??
          utilities.find((u) => u.id === "water")?.amount ??
          0,
        gas: totals.gas ?? utilities.find((u) => u.id === "gas")?.amount ?? 0,
        fuel:
          totals.fuel ?? utilities.find((u) => u.id === "fuel")?.amount ?? 0,
        grocery:
          totals.grocery ??
          utilities.find((u) => u.id === "grocery")?.amount ??
          0,
      };
      ...
      const totalAll = spendingArr.reduce((s, it) => s + (Number(it.amount) || 0), 0);
      const spendingComputed = spendingArr.map((it) => ({
        ...it,
        percent: totalAll > 0 ? Math.round((it.amount / totalAll) * 100) : 0,
      }));
      ...
      const mapPredToBillDate = (preds) =>
        (Array.isArray(preds) ? preds : []).map((p) => ({
          ...p,
      ...
      const computeForLatest = (latestBill, predictions) => {
        ...
        const scanned = Number(latestBill.cost || 0);
        const match = matchPrediction(latestBill, predictions);
        const predicted = match?.predictedCost ?? scanned * 1.1;
        const diff = predicted - scanned;
        return { scanned, predicted, diff };
      };
      ...
      const totalSpent = e.scanned + w.scanned;
      const savedAmount = [e.diff, w.diff].reduce((s, v) => s + (v > 0 ? v : 0), 0);
      ...
      console.log("homeData fatal error:", err?.response?.data || err.message);
    }
  };
