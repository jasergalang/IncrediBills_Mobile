import React from "react";
import { View, Text } from "react-native";

export default function ComparisonChart({ bill }) {
    const maxValue = Math.max(bill.scannedQuantity, bill.predictedQuantity);
    const currentPercent = (bill.scannedQuantity / maxValue) * 100;
    const predictedPercent = (bill.predictedQuantity / maxValue) * 100;

    return (
        <View className="px-4 pb-4">
            <Text className="text-base font-bold text-slate-900 mb-3">
                Usage Comparison
            </Text>
            <View className="bg-white rounded-2xl p-5 border border-slate-200">
                <View className="mb-4">
                    <View className="flex-row items-center justify-between mb-2">
                        <Text className="text-sm font-semibold text-slate-700">
                            Current Month
                        </Text>
                        <Text className="text-sm font-bold text-green-400">
                            {bill.scannedQuantity} 
                        </Text>
                    </View>
                    <View className="h-3 bg-slate-100 rounded-full overflow-hidden">
                        <View
                            className="h-full bg-green-200 rounded-full"
                            style={{ width: `${currentPercent}%` }}
                        />
                    </View>
                </View>

                <View>
                    <View className="flex-row items-center justify-between mb-2">
                        <Text className="text-sm font-semibold text-slate-700">
                            Predicted Next Month
                        </Text>
                        <Text className="text-sm font-bold text-green-600">
                            {bill.predictedQuantity} 
                        </Text>
                    </View>
                    <View className="h-3 bg-slate-100 rounded-full overflow-hidden">
                        <View
                            className="h-full bg-green-500 rounded-full"
                            style={{ width: `${predictedPercent}%` }}
                        />
                    </View>
                </View>
            </View>
        </View>
    );
}