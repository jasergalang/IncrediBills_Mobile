...
export const updateMiscellaneousBill = createAsyncThunk(
  "miscellaneous/update",
  async ({ billId, updatedData }, { dispatch, rejectWithValue }) => {
    try {
      const data = await updateMiscellaneousBillApi(billId, updatedData);
      dispatch(fetchMiscellaneousBills());
      return data;
    } catch (err) {
      const errorMessage =
...
const miscellaneousSlice = createSlice({
  ...
  reducers: {
    ...
    clearMiscellaneousError: (state) => {
      state.error = null;
    },
  },
  extraReducers: (builder) => {
    builder
      // ── fetchMiscellaneousBills ────────────────────────────────────────────
      .addCase(fetchMiscellaneousBills.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      ...
      // ── updateMiscellaneousBill ────────────────────────────────────────────
      .addCase(updateMiscellaneousBill.pending, (state) => {
        state.updating = true;
        state.error = null;
      })
      .addCase(updateMiscellaneousBill.fulfilled, (state) => {
        state.updating = false;
        state.error = null;
      })
      .addCase(updateMiscellaneousBill.rejected, (state, action) => {
        state.updating = false;
        state.error = action.payload;
      })

      // ── fetchMiscellaneousBillDetails ──────────────────────────────────────
      .addCase(fetchMiscellaneousBillDetails.pending, (state) => {
        state.detailsLoading = true;
        state.error = null;
      })
...
export const {
  removeMiscellaneousBillLocal,
  clearRecommendations,
  clearMiscellaneousError,
