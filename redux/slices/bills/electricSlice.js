...
export const updateElectricBill = createAsyncThunk(
  "electric/update",
  async ({ billId, updatedData }, { dispatch, rejectWithValue }) => {
    try {
      const data = await updateElectricBillApi(billId, updatedData);
      dispatch(fetchElectricBills());
      return data;
    } catch (err) {
      const errorMessage =
...
const electricSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ── updateElectricBill ─────────────────────────────────────────────────
      .addCase(updateElectricBill.pending, (state) => {
        state.updating = true;
        state.error = null;
      })
      .addCase(updateElectricBill.fulfilled, (state) => {
        state.updating = false;
        state.error = null;
      })
      .addCase(updateElectricBill.rejected, (state, action) => {
        state.updating = false;
        state.error = action.payload;
      })

      // ── fetchElectricBillDetails ───────────────────────────────────────────
      .addCase(fetchElectricBillDetails.pending, (state) => {
        state.detailsLoading = true;
        state.error = null;
      })
      .addCase(fetchElectricBillDetails.fulfilled, (state, action) => {
        ...
        state.selectedBill = {
          ...
          predictedConsumption:
            matchedPrediction?.predictedConsumption || bill.consumption * 1.1,
          predictedDate: matchedPrediction?.predictedDate || null,
        };
        state.error = null;
      })
...
export const { removeElectricBillLocal, clearRecommendations, clearElectricError } =
