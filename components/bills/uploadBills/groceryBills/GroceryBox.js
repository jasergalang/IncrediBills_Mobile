...
export default function GroceryBox({ pickImage, selectedImageUri, onRemoveImage }) {
  if (selectedImageUri) {
    return (
      <View className="rounded-2xl bg-white mb-4 overflow-hidden shadow-sm">
        ...
        <TouchableOpacity
          onPress={pickImage}
          className="bg-green-50 p-3 items-center border-t border-green-100"
          activeOpacity={0.7}
        >
          <Text className="text-green-600 font-semibold">Change Image</Text>
        </TouchableOpacity>
      </View>
    );
  }
