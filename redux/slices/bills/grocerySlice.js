...
export const updateGroceryBill = createAsyncThunk(
  "grocery/update",
  async ({ billId, updatedData }, { dispatch, rejectWithValue }) => {
    try {
      const data = await updateGroceryBillApi(billId, updatedData);
      dispatch(fetchGroceryBills());
      return data;
    } catch (err) {
      const errorMessage =
...
const grocerySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ── updateGroceryBill ──────────────────────────────────────────────────
      .addCase(updateGroceryBill.pending, (state) => {
        state.updating = true;
        state.error = null;
      })
      .addCase(updateGroceryBill.fulfilled, (state) => {
        state.updating = false;
        state.error = null;
      })
      .addCase(updateGroceryBill.rejected, (state, action) => {
        state.updating = false;
        state.error = action.payload;
      })

      // ── fetchGroceryBillDetails ────────────────────────────────────────────
      .addCase(fetchGroceryBillDetails.pending, (state) => {
        state.detailsLoading = true;
        state.error = null;
      })
      .addCase(fetchGroceryBillDetails.fulfilled, (state, action) => {
        ...
        state.selectedBill = {
          ...
          predictedQuantity:
            matchedPrediction?.predictedQuantity || bill.quantity * 1.1,
          predictedDate: matchedPrediction?.predictedDate || null,
        };
        state.error = null;
      })
...
export const { removeGroceryBillLocal, clearRecommendations, clearGroceryError } =
